...
### Feature drop — Store profile + Tiered Expiry Alerts + Bottle Loader + Parallax (2026-06-21)
  - **Store profile (backend)**: new `Store` collection with `name`, `owner_name`, `manager_name`, `location`, `currency`, `tagline`. Seeded on startup (idempotent) as "FreshTrack Bazaar". Endpoints: `GET /api/store` (any signed-in user) and `PUT /api/store` (admin only).
  - **Store profile (frontend)**: `StoreContext` provider supplies the live store to every component. The dashboard sidebar now displays Store Name + tagline + Owner / Manager lines under the user chip. The Overview page leads with a prominent glass header card showing the Store Name (XL display), tagline, Owner, Manager, Location. Settings page (admin-only) has a full "Store profile" editor.
  - **Tiered expiry alerts**: replaced the legacy `near_expiry`/`critical`/`expired` alert kinds with **six tiers + expired** — `expiry_tier_3h`, `expiry_tier_6h`, `expiry_tier_24h`, `expiry_tier_3d`, `expiry_tier_1w`, `expiry_tier_1m`, `expiry_expired`. Each alert's `meta` carries `tier_label`, `recommendation`, `remaining`, `product_name`, `batch_number`.
  - **Sales recommendations**: each tier has a tailored action — 3h: "apply heavy discount, bundle or donate"; 6h: "clear stock now: 40-60% off"; 24h: "priority sale, 25%+ discount today"; 3d: "move to promotional display"; 1w: "10-15% discount, combo-pack"; 1m: "plan promotional campaign"; expired: "pull from shelf, document loss".
  - **Dedup**: `/api/alerts/scan` now skips alerts where the same (product_id, kind) has already been emitted in the last 12 hours, so re-scans never spam.
  - **Popup notifications (frontend)**: new `<ExpiryNotifier>` component polls `/alerts` every 15s and shows up to 3 sliding glass-frosted popups bottom-right of the dashboard, each with product name, tier badge, remaining-time, and the action recommendation. Dismissed alerts are marked read on the server; seen IDs are remembered locally to avoid re-popping.
  - **Bottle-flip loader**: replaces the old "Loading workspace…" text screen — a SVG bottle drops, flips 720°, lands standing on a soft shadow, and the dotted "Loading workspace…" label types in. Used by the `<Protected>` route while `/auth/me` is verifying the cookie.
  - **Parallax zoom on landing**: hero now uses `useScroll` + `useTransform` to scale and translate (a) the floating scan card (1.0 → 1.18× scale + drift), (b) the background radial blobs (1.45× and 1.55×), and (c) the main headline (1.05× scale + drift) as the user scrolls — giving a cinematic depth feel without disturbing readability.
  - **Backend**: ran 58/58 tests green (43 regression + 15 new). Inventory edit/delete unchanged. Camera UX (start/stop, file upload fallback) unchanged — still works on desktop / laptop / mobile / tablet via the existing `<CameraPanel>` + `<input type=file accept="image/*">` fallback.
